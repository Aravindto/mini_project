...
<body>
<h1 align="center">7.1.17 - Promotion of Universal values</h1>
<?php
...
if(isset($_POST['submit']))
		{
			 ...
			 $to=$_POST['to'];
			 $partic=$_POST['part'];
			 $query="insert into promotion values ('','$qm','$year','$actv','$from','$to','$partic')";
			  $q=mysql_query($query)or die(mysql_error());
			  header("location:7.1.17_des.php");
			  echo "status:saved";
			 }	?>

<form class="form-wrapper-2 cf" action="7.1.17_rep.php" method="post">
<input type="text" placeholder="Search here..." name="search1">
<button type="submit" name="search">Search</button>
</form>
<table class="tb"><tr><td>
<form method="post" name="fm" action="7.1.17_rep.php">

<input type="submit" name="pdf" value="PDF" class="pdf"/></form></td><td>
<form method="post" name="fm" action="excel.php">

<input type="submit" name="excel7117" value="EXCEL" class="excel"/>
</form></td></tr></table></body>
<?php
function fetch_data()
{
$output='';
$env="select * from promotion";
$q1=mysql_query($env);
 while($rs=mysql_fetch_array($q1))
{
$output .= "<tr><td>".$rs['pmno']."</td>
	<td>".$rs['year']."</td>
	<td>".$rs['activity']."</td>
	<td>".$rs['frm']."</td>
	<td>".$rs['dto']."</td>
	<td>".$rs['parti']."</td>
	<td>
                                    <form method=post action='7.1.17_des.php' enctype='multipart/form-data'>
                                    <input type=hidden name=pmno id=lig_no value=".$rs["pmno"]." />
                                    <input type=hidden name=action value=edit >
                                    <input type=submit value=Edit class='btn'>
                                    </form> 
                                    </td>
                               <td>
                         <form method=POST action='7.1.17_rep.php' onSubmit='return ConfirmDelete();'enctype='multipart/form-data'>
                                    <input type=hidden name=pmno id=lig_no value=".$rs["pmno"]." />
...
{
$sear=$_POST['search1'];
$var="select * from promotion where pmno='$sear'";
...
<thead><tr><th>sl_no</th>
    ...
    <th>To</th>
    <th>Number of participants</th></tr></thead>";
 	echo "<tr><td>".$fetch['pmno']."</td>";
	echo "<td>".$fetch['year']."</td>";
 	echo "<td>".$fetch['activity']."</td>";
	echo"<td>".$fetch['frm']."</td>";
	echo"<td>".$fetch['dto']."</td>";
	echo"<td>".$fetch['parti']."</td>";
				echo "<td>";
                                    echo "<form method=post action='7.1.17_des.php' enctype='multipart/form-data'>";
                                    echo "<input type=hidden name=pmno id=lig_no value=".$fetch["pmno"]." />";
                                    echo "<input type=hidden name=action value=edit >";
                                    echo "<input type=submit value=Edit class='btn'>";
                                    echo "</form>"; 
                                    echo "</td>";
                                    echo "<td>";
                    echo "<form method=POST action='7.1.17_rep.php' onSubmit='return ConfirmDelete();'enctype='multipart/form-data'>";
                                    echo "<input type=hidden name=pmno id=lig_no value=".$fetch["pmno"]." />";
...
$to=$_POST['to'];
$partic=$_POST['part'];
$up="UPDATE promotion set qm_index='$qm',year='$year',activity='$activ',frm='$frm',dto='$to',parti='$partic' where pmno='$pmno_no'";
...
<?php
      ...
      if(isset($_POST["action"]) and $_POST["action"]=="delete")
        {
	$pmno_no = (isset($_POST["pmno"])? $_POST["pmno"] : '');
	 $sql = "delete from promotion where pmno = '$pmno_no'";
...
function title()
{
	...
	$qmn=mysql_fetch_array($qm1);*/
	$a=date('y/m/d');
	$h .= "date:".$a."<br>7.1.17 - Promotion of Universal values<br>
<br>";	
return $h;
}
if(isset($_POST['pdf']))
{
   ...
   public function Header() {
    ...
    public function Footer() {
      ...
      $obj_pdf->SetTitle("7.1.17 AVG NO OF QUALITY INITIATIVES");  
